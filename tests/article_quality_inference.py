...
# ✅ 加载模型和 Tokenizer
model_name = "data/Qwen2.5-0.5B-Instruct"  # 替换为你的模型路径或模型名称
...
for example in dataset['train']:
    input_chat = example["input_only_text"]
    ...
    for i, token_id in enumerate(generated_tokens):
        ...
        eos_token_prob = probs[0, i, tokenizer.eos_token_id].item()  # Get the probability of EOS token
        print(f"Token: {token_str}, Probability: {token_prob:.6f}, EOS Token Probability: {eos_token_prob:.6f}")
